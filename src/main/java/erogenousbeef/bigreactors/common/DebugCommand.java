...
public class DebugCommand extends CommandBase {
    ...
    private void printReactorInteriorData(ICommandSender sender, boolean fluid, String name, ReactorInteriorData data) {
        ...
        sender.addChatMessage(new TextComponentString(String.format("Reactor moderator data: %1$s (%2$s)",
                name, fluid ? "fluid" : "block")));

        if (null == data) {

            sender.addChatMessage(new TextComponentString("Moderator not found!"));

        } else {

            sender.addChatMessage(new TextComponentString(String.format(" - absorption %1$f", data.absorption)));
            sender.addChatMessage(new TextComponentString(String.format(" - heatEfficiency %1$f", data.heatEfficiency)));
            sender.addChatMessage(new TextComponentString(String.format(" - moderation %1$f", data.moderation)));
            sender.addChatMessage(new TextComponentString(String.format(" - heatConductivity %1$f", data.heatConductivity)));
        }
    }
}